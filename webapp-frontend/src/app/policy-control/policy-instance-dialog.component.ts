...
export class PolicyInstanceDialogComponent implements OnInit, AfterViewInit {
    ...
    jsonFormValid = false; // Not needed
    jsonFormStatusMessage = 'Loading form...';
    jsonFormSchemaObject: any;
    jsonFormData: any = {};
    ...
    onSubmit(data: any) {
        ...
        this.dataService.putPolicy(this.policyTypeId, this.policyInstanceId, policyJson).subscribe(
            {
                next(value) {
                    console.log('updatePolicy OK ' + value);
                },
                error(error) {
    ...
    get schemaAsString() {
        return JSON.stringify(this.jsonFormSchemaObject, null, 2);
    }

    get jsonAsString() {
        return JSON.stringify(this.jsonFormData, null, 2);
    }
    ...
    private parseJson() {
        try {
            this.jsonFormSchemaObject = JSON.parse(this.data.createSchema);
            if (this.data.instanceJson != null) {
                this.jsonFormData = JSON.parse(this.data.instanceJson);
            }
