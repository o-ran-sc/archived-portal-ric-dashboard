...
@Profile("!test")
public class CassIngressConfiguration {
	...
	@Autowired
	public CassIngressConfiguration( //
		...
		caasIngressAuxUrl = new DefaultUriBuilderFactory(auxUrlPrefix.trim()).builder().path(auxUrlSuffix.trim()).build()
				.normalize().toString();
		caasIngressPltUrl = new DefaultUriBuilderFactory(pltUrlPrefix.trim()).builder().path(pltUrlSuffix.trim()).build()
				.normalize().toString();
