...
import org.oransc.ric.a1controller.client.model.PolicyTypeSchema;
...
public class A1Controller {
	...
	public static final String CONTROLLER_PATH = DashboardConstants.ENDPOINT_PREFIX + "/a1";
	// Endpoints
	...
	public Object getAllPolicyTypes(HttpServletResponse response) {
		...
		List<Integer> policyTypeIds = a1ControllerApi.a1ControllerGetAllPolicyTypes();
		PolicyTypes policyTypes = new PolicyTypes();
		for (Integer policyTypeId : policyTypeIds) {
			PolicyTypeSchema policyTypeSchema = a1ControllerApi.a1ControllerGetPolicyType(policyTypeId);
			PolicyType type = new PolicyType(policyTypeSchema.getPolicyTypeId(), policyTypeSchema.getName(),
					policyTypeSchema.getDescription(), policyTypeSchema.getCreateSchema().toString());
	...
	public Object getPolicyInstances(@PathVariable(POLICY_TYPE_ID_NAME) String policyTypeIdString) {
		...
		List<String> instancesForType = a1ControllerApi.a1ControllerGetAllInstancesForType(policyTypeId);
		PolicyInstances instances = new PolicyInstances();
		for (String instanceId : instancesForType) {
			Object policyInstance = a1ControllerApi.a1ControllerGetPolicyInstance(policyTypeId, instanceId);
	...
	@GetMapping(POLICY_TYPES_METHOD + "/{" + POLICY_TYPE_ID_NAME + "}/" + POLICIES_NAME + "/{" + POLICY_INSTANCE_ID_NAME
			+ "}")
	@Secured({ DashboardConstants.ROLE_ADMIN, DashboardConstants.ROLE_STANDARD })
	public Object getPolicyInstance(@PathVariable(POLICY_TYPE_ID_NAME) String policyTypeIdString, @PathVariable(POLICY_INSTANCE_ID_NAME) String policyInstanceId) {
		logger.debug("getPolicyInstance {}:{}", policyTypeIdString, policyInstanceId);
		Integer policyTypeId = Integer.decode(policyTypeIdString);
		return a1ControllerApi.a1ControllerGetPolicyInstance(policyTypeId, policyInstanceId);
	}
	...
	public void putPolicyInstance(@PathVariable(POLICY_TYPE_ID_NAME) String policyTypeIdString,
		...
		logger.debug("putPolicyInstance typeId: {}, instanceId: {}, instance: {}", policyTypeIdString, policyInstanceId, instance);
		Integer policyTypeId = Integer.decode(policyTypeIdString);
		a1ControllerApi.a1ControllerCreateOrReplacePolicyInstance(policyTypeId, policyInstanceId, instance);
	}
	...
	public void deletePolicyInstance(@PathVariable(POLICY_TYPE_ID_NAME) String policyTypeIdString,
		...
		Integer policyTypeId = Integer.decode(policyTypeIdString);
		a1ControllerApi.a1ControllerDeletePolicyInstance(policyTypeId, policyInstanceId);
	}
}
