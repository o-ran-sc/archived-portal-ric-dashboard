...
export class AppMgrService {
  ...
  getDeployed(): Observable<XMDeployedApp[]> {
    return this.httpClient.get<XMDeployedApp[]>(this.instanceSelectorService.getBasePath() + '/xapps');
  }

  deployXapp(name: string): Observable<HttpResponse<Object>> {
    const xappInfo: XMXappInfo = { name: name };
    return this.httpClient.post((this.instanceSelectorService.getBasePath() + '/xapps'), xappInfo, { observe: 'response' });
  }

  undeployXapp(name: string): Observable<HttpResponse<Object>> {
    return this.httpClient.delete((this.instanceSelectorService.getBasePath() + '/xapps'+ '/' + name), { observe: 'response' });
  }
  ...
  putConfig(config: any): Observable<HttpResponse<Object>> {
    return this.httpClient.put((this.instanceSelectorService.getBasePath() + '/config' ), config, { observe: 'response' });
  }
