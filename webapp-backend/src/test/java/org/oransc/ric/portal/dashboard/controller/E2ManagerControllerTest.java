...
public class E2ManagerControllerTest extends AbstractControllerTest {
	...
	public void ranDetailsTest() {
		...
		logger.info("Invoking {}", uri);
		ResponseEntity<List<RanDetailsTransport>> response = testRestTemplateUser().exchange(uri, HttpMethod.GET, null,
	...
	public void nodebListTest() {
		...
		logger.info("Invoking {}", uri);
		ResponseEntity<List<NodebIdentity>> response = testRestTemplateUser().exchange(uri, HttpMethod.GET, null,
	...
	public void nodebStatusTest() {
		...
		logger.info("Invoking {}", uri);
		GetNodebResponse response = testRestTemplateUser().getForObject(uri, GetNodebResponse.class);
	...
	public void endcSetupTest() {
		...
		ResponseEntity<Void> voidResponse = testRestTemplateAdmin().exchange(uri, HttpMethod.POST, entity, Void.class);
		Assertions.assertTrue(voidResponse.getStatusCode().is2xxSuccessful());
	}
