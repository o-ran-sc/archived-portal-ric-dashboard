...
public class PortalLoginFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		if (request.getRequestURI().startsWith(restApiURI)) {
			...
			logger.trace("doFilter: delegating auth to REST servlet for request {}", request);
			chain.doFilter(request, response);
		} else {
			// Need to authenticate the request
			if (authManager.valdiateEcompSso(request) == null) {
				// This is JUST the portion of the path after the context
				String appPath = request.getRequestURI().substring(request.getContextPath().length() + 1)
						+ (request.getQueryString() != null ? ("?" + request.getQueryString()) : "");
				String redirectURL = SSOUtil.getECOMPSSORedirectURL(request, response, appPath);
				logger.trace("doFilter: unauthorized; app path is {}, redirecting to {}", appPath, redirectURL);
