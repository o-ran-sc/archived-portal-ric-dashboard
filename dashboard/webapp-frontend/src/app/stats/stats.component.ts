...
import { StatsResponse } from '../interfaces/e2-mgr.types';
...
export class StatsComponent implements OnInit {
    ...
    editAppMetrics(stats?) {
      ...
      viewAppMetrics(stats?) {
        this.statsservice.getAppMetricsById(this.instanceKey, stats.statsDetails.appId)  .subscribe((res: StatsResponse) => {
      ...
      deleteAppMetrics(stats?) {
        this.statsservice.deleteAppMetrics(this.instanceKey, stats.statsDetails.appId).subscribe((res: any) => {
          //let tabNotThere:boolean = true;
          /*if (stats.length <= 0) {
            console.log('No xApp metrics to delete');
          }*/
          if (res) {
            for(let i=0; i<this.tabs.length; i++){
              if (this.tabs[i].instanceKey === res.body.instanceKey && this.tabs[i].statsDetails.appId == res.body.statsDetails.appId) {
