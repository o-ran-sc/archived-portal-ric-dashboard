...
public class StatsManager {
	 ...
	 */
	public synchronized IDashboardResponse createStats(String instanceKey, StatsDetailsTransport statsSetupRequest,
			HttpServletResponse response) {
		logger.debug("createStats: appId {}, instanceKey {}", statsSetupRequest.getAppId(), instanceKey);
		/*
		 * if (users.contains(user)) throw new PortalAPIException("User exists: " +
		 * user.getLoginId()); users.add(user); try { saveUsers(); } catch (Exception
		 * ex) { throw new PortalAPIException("Save failed", ex); }
		 */
		...
		try {
			...
			return new ErrorTransport(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Save stats failed");
		}
	...
	public synchronized IDashboardResponse deleteStats(String instanceKey, int appId, HttpServletResponse response) {
		logger.debug("deleteStats: appId {}, instanceKey {}", appId, instanceKey);
		boolean deleteStatsObjectFound = false;

		for (Stats st : stats) {
			if (st.getInstanceKey().equals(instanceKey) && st.getStatsDetails().getAppId() == appId) {
				deleteStatsObjectFound = stats.remove(st);
				try {
					saveStats();
					response.setStatus(HttpServletResponse.SC_OK);
					return st;
					...
					stats.add(st);
					response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
					logger.debug("deleteUser: failed to delete stats from local file store ", appId, instanceKey);
				}
		...
		if (!deleteStatsObjectFound) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
			logger.debug("deleteUser: no match on app id {} of instance key {}", appId, instanceKey);
		}
		return null;
