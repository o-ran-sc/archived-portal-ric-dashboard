...
public class PortalApiMockConfiguration {
	...
	public ServletRegistrationBean<PortalRestAPIProxy> portalApiProxy() {
		PortalRestAPIProxy mockProxy = mock(PortalRestAPIProxy.class);
		final ServletRegistrationBean<PortalRestAPIProxy> servlet = new ServletRegistrationBean<>(mockProxy,
				PortalApiConstants.API_PREFIX + "/*");
		servlet.setName("PortalRestApiMockProxy");
		return servlet;
	}
	...
	@Bean
	public DashboardUserManager portalUserManager() throws Exception {
		DashboardUserManager mockManager = mock(DashboardUserManager.class);
		doAnswer(inv -> {
			logger.debug("createUser");
			return null;
		}).when(mockManager).createUser(any(EcompUser.class));
		doAnswer(inv -> {
			logger.debug("updateUser");
			return null;
		}).when(mockManager).updateUser(any(String.class), any(EcompUser.class));
		return mockManager;
	}
