...
public class PortalApiConfiguration {
	...
	/*
	 * Instantiates the EPSDK-FW-compatible web filter. Needed because Spring-Boot
	 * does not automatically process @WebFilter annotations.
	 * 
	 * @return Filter registration bean for a Dashboard login filter.
	 */
	@Bean
	@DependsOn("portalAuthManager")
	public FilterRegistrationBean<PortalLoginFilter> portalLoginFilter()
			throws ClassNotFoundException, InstantiationException, IllegalAccessException, IOException {
		// Is this the right way to get the auth bean?
		PortalLoginFilter filter = new PortalLoginFilter(portalAuthManager());
		final FilterRegistrationBean<PortalLoginFilter> filterBean = new FilterRegistrationBean<>(filter);
		filterBean.addUrlPatterns("/*");
		filterBean.setName("PortalLoginFilter");
		return filterBean;
	}
	...
	public ServletRegistrationBean<PortalRestAPIProxy> portalApiProxy() {
		...
		servletBean.setName("PortalRestApiProxy");
		return servletBean;
	}
}
