...
public class PortalAuthManager {
	...
	public String valdiateEcompSso(HttpServletRequest request) {
		...
		if (ep == null) {
			logger.debug("valdiateEcompSso: EP cookie not found, returning null");
			return null;
		}
		logger.trace("valdiateEcompSso: found EP service cookie!");
		Cookie user = getCookie(request, userIdCookieName);
		if (user == null) {
			logger.debug("valdiateEcompSso: user cookie not found, returning null");
			return null;
		}
		logger.trace("valdiateEcompSso: found user cookie!");
		String userid = null;
		try {
