...
export class StatsComponent implements OnInit {
    ...
    editAppMetrics(stats?) {
      ...
      viewAppMetrics(stats?) {
        this.statsservice.getAppMetricsById(this.instanceKey, stats.statsIdentity.appId)  .subscribe((res: StatsDetails) => {
          this.metricsUrl = this.sanitize.bypassSecurityTrustResourceUrl(res.statsStatus.metricUrl);
          ...
          if (this.tabs.length <= 0) {
            ...
            for(let i=0; i<this.tabs.length; i++){
              if (this.tabs[i].statsIdentity.appId == res.statsIdentity.appId) {
                this.tabs[i].statsStatus.appName = res.statsStatus.appName;
                this.tabs[i].statsStatus.metricUrl = res.statsStatus.metricUrl;
