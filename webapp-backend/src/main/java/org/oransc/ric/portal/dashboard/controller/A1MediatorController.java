...
public class A1MediatorController {
	...
	public Object getPolicy(@PathVariable(DashboardConstants.RIC_INSTANCE_KEY) String instance,
			@PathVariable(PP_POLICIES) String policyName) {
		logger.debug("getPolicy instance {} policy {}", instance, policyName);
		return a1MediatorClientBuilder.getA1MediatorApi(instance).a1ControllerGetHandler(policyName);
	}

	@ApiOperation(value = "Puts the specified policy to the A1 Mediator")
	@PutMapping(POLICY_METHOD_PATH)
	@Secured({ DashboardConstants.ROLE_ADMIN })
	public void putPolicy(@PathVariable(DashboardConstants.RIC_INSTANCE_KEY) String instance,
			@PathVariable(PP_POLICIES) String policyName, @ApiParam(value = "Policy body") @RequestBody String policy, //
			HttpServletResponse response) {
		logger.debug("putPolicy instance {} name {} value {}", instance, policyName, policy);
		A1MediatorApi api = a1MediatorClientBuilder.getA1MediatorApi(instance);
