...
public class AppManagerControllerTest extends AbstractControllerTest {
	...
	public void appListTest() {
		...
		logger.info("Invoking {}", uri);
		DashboardDeployableXapps apps = testRestTemplateUser().getForObject(uri, DashboardDeployableXapps.class);
	...
	public void appStatusesTest() {
		...
		logger.info("Invoking {}", uri);
		AllDeployedXapps apps = testRestTemplateUser().getForObject(uri, AllDeployedXapps.class);
		Assertions.assertFalse(apps.isEmpty());
	}
	...
	public void appStatusTest() {
		...
		logger.info("Invoking {}", uri);
		Xapp app = testRestTemplateUser().getForObject(uri, Xapp.class);
	...
	public void deployAppTest() {
		...
		Xapp app = testRestTemplateAdmin().postForObject(uri, info, Xapp.class);
		Assertions.assertFalse(app.getName().isEmpty());
	}
	...
	public void undeployAppTest() {
		URI uri = buildUri(null, AppManagerController.CONTROLLER_PATH, AppManagerController.XAPPS_METHOD, "app");
		logger.info("Invoking {}", uri);
		ResponseEntity<Void> voidResponse = testRestTemplateAdmin().exchange(uri, HttpMethod.DELETE, null, Void.class);
		Assertions.assertTrue(voidResponse.getStatusCode().is2xxSuccessful());
	}
	...
	public void getConfigTest() {
		...
		logger.info("Invoking {}", uri);
		AllXappConfig config = testRestTemplateUser().getForObject(uri, AllXappConfig.class);
	...
	public void createConfigTest() {
		...
		XAppConfig response = testRestTemplateAdmin().postForObject(uri, newConfig, XAppConfig.class);
	...
	public void deleteConfigTest() {
		...
		ResponseEntity<Void> voidResponse = testRestTemplateAdmin().exchange(uri, HttpMethod.DELETE, entity,
				Void.class);
		Assertions.assertTrue(voidResponse.getStatusCode().is2xxSuccessful());
	}
